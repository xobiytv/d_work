...
const ExploreCard = ({ id, imgUrl, title, index, active, handleClick }) => (
  <motion.div
    ...
    className={`relative ${
      active === id ? 'lg:flex-[3.5] flex-[10] ' : 'lg:flex-[1] flex-[5]'
  ...
  >
    ...
    {active !== id ? (
      <h3
        className="font-semibold sm:text-[26px] text-[18px] text-[#fff] absolute z-0
      ...
      <div className="absolute bottom-0 p-8  justify-start w-full  bg-[rgba(0,0,0, 0.5)] rounded-b-[24px]">
        ...
        <p className="font-normal text-[16px] leading-[20px] text-[#fff] uppercase ">
          Enter the Metaverse
        </p>
        <h2 className="mt-[24px] font-semibold sm:text-[32px] text-[24px] text-[#fff] ">
            {title}
          </h2>
      </div>
    )}
